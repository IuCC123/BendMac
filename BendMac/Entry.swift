...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification:Notification) {
        ...
        let toggle=menu.addItem(withTitle:"Enable / pause effect",action:#selector(toggleEffect),keyEquivalent:""); toggle.target=self
        let settingsItem=menu.addItem(withTitle:"Settings…",action:#selector(openSettings),keyEquivalent:","); settingsItem.target=self
        let preview=menu.addItem(withTitle:"Play preview",action:#selector(playPreview),keyEquivalent:""); preview.target=self
        menu.addItem(.separator())
        menu.addItem(withTitle:"Quit BendMac",action:#selector(NSApplication.terminate(_:)),keyEquivalent:"q")
    ...
    @objc func playPreview() { openSettings(); model.playPreview() }
    @objc func openSettings() {
        if settings == nil {
            let window=NSWindow(contentRect:NSRect(x:0,y:0,width:850,height:750),styleMask:[.titled,.closable,.miniaturizable,.fullSizeContentView],backing:.buffered,defer:false)
            window.title="BendMac"; window.titlebarAppearsTransparent=true; window.titleVisibility = .hidden
            window.isReleasedWhenClosed=false
            window.contentView=NSHostingView(rootView:SettingsView(model:model).padding(.top,26))
            window.center(); window.delegate=self; settings=window
        }
...
if let index=CommandLine.arguments.firstIndex(of:"--render-proof"), CommandLine.arguments.count > index+1 {
    ...
    do {
        ...
        for frame in 0...126 {
            ...
            try renderer.exportPreview(p,to:folder.appendingPathComponent(String(format:"frame-%03d.png",frame)))
        }
