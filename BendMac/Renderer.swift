...
final class BendRenderer: NSObject, MTKViewDelegate {
    ...
    init(frames: FrameStore, preview: CGImage? = nil) throws {
        ...
        guard let texture = device.makeTexture(descriptor: td) else {
            throw NSError(domain: "Preview texture allocation failed", code: 2)
        }
    ...
    static func previewImage() -> CGImage {
        ...
        let image = NSImage(size: size, flipped: false) { rect in
            ...
            let snow = NSBezierPath()
            snow.move(to: .zero)
            snow.line(to: NSPoint(x: 0, y: 185))
            snow.curve(
                to: NSPoint(x: 1280, y: 315), controlPoint1: NSPoint(x: 500, y: 320),
                controlPoint2: NSPoint(x: 600, y: 30))
            snow.line(to: NSPoint(x: 1280, y: 0))
            snow.close()
            NSColor(calibratedRed: 0.77, green: 0.83, blue: 0.89, alpha: 1).setFill()
            snow.fill()
            NSColor(calibratedWhite: 0.93, alpha: 1).setFill()
            NSBezierPath(ovalIn: NSRect(x: 950, y: 570, width: 70, height: 70)).fill()
            let centered = NSMutableParagraphStyle()
            centered.alignment = .center
            ("Thursday, September 10" as NSString).draw(
                in: NSRect(x: 0, y: 630, width: 1280, height: 30),
                withAttributes: [
                    .font: NSFont.systemFont(ofSize: 23, weight: .medium), .foregroundColor: NSColor.white,
                    .paragraphStyle: centered,
                ])
            ("9:41" as NSString).draw(
                in: NSRect(x: 0, y: 488, width: 1280, height: 145),
                withAttributes: [
                    .font: NSFont.systemFont(ofSize: 126, weight: .light), .foregroundColor: NSColor.white,
                    .paragraphStyle: centered,
                ])
            return true
        }
