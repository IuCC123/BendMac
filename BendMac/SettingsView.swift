...
struct SettingsView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 6) {
                Label("BendMac", systemImage: "macbook").font(.title3.weight(.semibold)).padding(.bottom, 25)
                    .padding(.horizontal, 12)
                sidebar("General", icon: "gearshape")
                Text("SETTINGS").font(.caption2.weight(.semibold)).foregroundStyle(.secondary).padding(
                    .top, 20
                ).padding(.leading, 12)
                sidebar("Appearance", icon: "circle.lefthalf.filled")
                Spacer()
                Label(
                    model.sensorAngle == nil ? "Sensor unavailable" : "Lid sensor connected",
                    systemImage: model.sensorAngle == nil ? "exclamationmark.circle" : "checkmark.circle.fill"
                )
                .font(.caption).foregroundStyle(model.sensorAngle == nil ? .orange : .green).padding(
                    .horizontal, 10)
                Text("Native. Local. Just a little bend.").font(.caption2).foregroundStyle(.secondary)
                    .padding(.horizontal, 10)
            }.padding(16).frame(width: 176).frame(maxHeight: .infinity).background(.ultraThinMaterial)
            Divider()
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    Label(tab, systemImage: tab == "Appearance" ? "circle.lefthalf.filled" : "gearshape")
    ...
    private func sidebar(_ title: String, icon: String) -> some View {
        Button {
            ...
            Label(title, systemImage: icon).font(.body.weight(.medium)).frame(
                maxWidth: .infinity, alignment: .leading
            ).padding(10)
                .background(
                    tab == title ? Color.accentColor.opacity(0.16) : .clear,
    ...
    @ViewBuilder private var appearance: some View {
        VStack(spacing: 0) {
            ZStack(alignment: .top) {
                MetalPreview(model: model).aspectRatio(1.6, contentMode: .fit).clipShape(
                ...
                VStack {
                    ...
                    Button {
                        ...
                        Image(systemName: model.previewPlaying ? "waveform" : "play.fill").font(.title2)
                            .frame(width: 48, height: 48).background(.ultraThinMaterial, in: Circle())
                    }.buttonStyle(.plain).help("Play a full fold and unfold preview").accessibilityLabel(
                        "Play fold animation")
                    Spacer()
                }
            }.background(.black, in: RoundedRectangle(cornerRadius: 18))
            UnevenRoundedRectangle(bottomLeadingRadius: 6, bottomTrailingRadius: 6).fill(
                Color(nsColor: .systemGray)
            ).frame(height: 12).padding(.horizontal, -15)
        ...
        HStack {
            ...
            Text("The preview uses the same Metal effect as your desktop.").font(.caption).foregroundStyle(
                .secondary)
            Spacer()
            Button("Reset") {
                model.perspective = 1
    ...
    private func styleCard(_ name: String, index: Int, description: String, icon: String) -> some View {
        Button {
            ...
            VStack(spacing: 8) {
                ...
                .background(
                    model.style == index ? Color.accentColor.opacity(0.1) : Color.primary.opacity(0.035),
                    in: RoundedRectangle(cornerRadius: 12)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 12).strokeBorder(
                        model.style == index ? Color.accentColor : .clear, lineWidth: 2))
    ...
    private func settingSlider(_ name: String, value: Binding<Double>) -> some View {
        HStack {
            ...
            Text("\(Int(value.wrappedValue*100))%").monospacedDigit().foregroundStyle(.secondary).frame(
    ...
    @ViewBuilder private var general: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(
                        model.enabled
                            ? "Your desktop is ready to bend" : "Give your desktop a little flexibility"
                    ).font(.headline)
            ...
            HStack {
                Button(
                    model.starting ? "Connecting…" : model.enabled ? "Pause effect" : "Enable desktop effect"
                ) {
        ...
        GroupBox("Lid behavior") {
            VStack(alignment: .leading, spacing: 18) {
                HStack {
                    ...
                    Spacer()
                    Text(model.sensorAngle.map { "\(Int($0))°" } ?? "Unavailable").monospacedDigit()
                        .foregroundStyle(.secondary)
                }
                if !model.followLid {
                    HStack {
                        Text("Desktop angle")
                        Slider(value: $model.manualAngle, in: 12...135).accessibilityLabel(
                            "Live desktop angle")
                ...
                HStack {
                    Text("Clear at")
                    Slider(value: $model.clearAngle, in: 80...135).accessibilityLabel(
                        "Lid angle at which desktop clears")
                    Text("\(Int(model.clearAngle))°").monospacedDigit().frame(width: 40)
                }
                Text("Above this angle your desktop returns to normal. Only the built-in display bends.")
        ...
        GroupBox("Screen Recording") {
            VStack(alignment: .leading, spacing: 12) {
                Text(
                    "macOS needs your permission to show a live view of the desktop. Frames stay in memory on this Mac; no audio is captured and nothing is saved or uploaded."
        ...
        Text(
            "BendMac · Free and open source. Inspired by Bendy.\nSwift • AppKit • ScreenCaptureKit • Metal\nThe lid sensor report is undocumented and may vary between macOS versions."
        ).font(.caption).foregroundStyle(.secondary)
    }
}
