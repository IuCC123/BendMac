...
let two=BendMath.smooth(current:BendMath.smooth(current:0,target:1,dt:1/60.0),target:1,dt:1/60.0)
check(abs(one-two)<1e-9,"Smoothing must not depend on refresh rate")
check(BendMath.smooth(current:0.4,target:0,dt:1/60.0)<0.4,"Direction changes must reverse immediately")
